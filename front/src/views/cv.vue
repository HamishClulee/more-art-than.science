<template>
    <div class="cv-con" id="print-vis-on">
        ...
        <div class="job-con" v-for="(v, i) in jobs" :key="i">
            ...
            <h3>{{ v.title }}</h3>
            <h5>{{ v.dates }}</h5>
            <p v-for="(p, x) in v.description" :key="x">{{ p }}</p>
            <div class="fin-con">~</div>
        </div>
    </div>
</template>
                ...
                jobs: [
                    {
                        ...
                        description: [
                            ...
                            `A booking, payment and calendar management app for a company that provides co-working and co-living spaces for digital nomads.`,
                            `I spent a lot of time working as a contract developer for a design and prototyping company attached to a large US health care firm. I rebuilt their frontend build system, introduced them to VueJs and built a lot of prototypes for them to on sell to their parent company.`,
